...
/*   Updated: 2024/04/03 12:30:21 by mcatalan@st      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	print_struct(t_table *table)
{
	...
	while (i < table->n_philo)
	{
		...
		printf(YELLOW"-Left fork: %d\n" RST, table->philo[i].l_fork);
		printf(YELLOW"-Right fork: %d\n" RST, table->philo[i].r_fork);
		printf(YELLOW"-Last meal time: %ld\n" RST, table->philo[i].l_meal);
